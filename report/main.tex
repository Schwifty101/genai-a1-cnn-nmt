...
source lengths, but that fluency does not imply faithfulness. For the
10-token source ``blessed are the meek for they shall inherit the
earth,'' the reference reads (glossed) ``blessed are those who are
meek, for they will inherit the earth,'' but the hypothesis glosses to
``blessed are those who are near to God'' --- correct Beatitude
register, wrong predicate. For the 30-token source ``and now also the
axe is laid unto the root of the trees therefore every tree which
bringeth not forth good fruit is hewn down and cast into the fire,''
the hypothesis glosses to ``and now I say to you, because of the
grapes, woe unto you, that none of you should [UNK]'' --- fluent,
generic Biblical Urdu with no relationship to the source's axe, tree,
or fire imagery. Both patterns recur throughout
\texttt{results/q2\_samples.csv}.

\input{tables/q2_lengthbucket.tex}
